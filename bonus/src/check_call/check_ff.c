...
static int temp_call(char *name, process_info_t *proc, long value)
{
    process_library_t *lib = get_lib(proc, value);

    call_stack_add(name, (lib == NULL) ? proc->exec_name : lib->name, value);
...
static int check_ff2_call(process_info_t *proc, unsigned char modrm)
{
    ...
    if (modrm % 8 == 4 && modrm != 0xD4) {
        ...
        return temp_call(name, proc, value);
    }
